...
- 🎙 **リアルタイム文字起こし** — ブラウザのマイク入力を発話の区切りごとに文字起こし。発話中は暫定テキストを逐次表示し、発話が終わると確定。一時停止・再開に対応し、停止後は TXT でダウンロード可能
- 📁 **ファイル文字起こし** — 音声・動画ファイル(mp3 / m4a / wav / mp4 / mov / mkv / webm など)をアップロードして文字起こし
- 🕒 **時刻タグ付き** — すべてのセグメントに `[HH:MM:SS]` 形式の時刻を付与。「発言の区切り」(短い/標準/長い)で表示・TXT の粒度を調整可能
- 📖 **用語リスト・コンテキスト** — 固有名詞や前提知識を事前入力して認識精度を向上。会議・案件ごとの**プリセット**として登録・共有可能
- 👥 **話者タグ** — 「話者1」「話者2」…の仮名で自動分離し、あとから氏名を入力して一括で振り分け
- 📤 **エクスポート** — TXT(時刻付き)/ SRT / VTT / JSON
- 📝 **要約・議事録の生成(任意)** — Ollama などのローカル LLM と連携し、文字起こしから要約・議事録を生成(設定しない限り無効。外部サービスには送信されません)
...
| `LLM_API_URL` | (空=無効) | ローカル LLM の OpenAI 互換 API ベース URL(下記「要約・議事録の生成」参照) |
...
**プリセット**: よく使う会議・案件ごとの用語リスト+コンテキストは、名前を付けてプリセット登録できます(例: 「開発定例」「顧客A案件」)。プリセットはサーバーの DB に保存されるため、社内公開時は利用者間で共有され、部署共通の用語集として運用できます。同名で保存すると上書きです。

## 話者タグ(話者分離)

文字起こし時にセグメントごとの声紋(話者埋め込み)を取り、同一セッション内でクラスタリングして「話者1」「話者2」…の仮名タグを自動付与します(sherpa-onnx + CAM++ モデル、約28MB。初回利用時に自動ダウンロードされ `model_cache` にキャッシュ)。

- 結果画面の「話者の氏名」欄に氏名を入力すると、その話者のセグメントすべてに一括で反映されます
- ファイル文字起こしでは「保存」すると氏名がサーバーに保存され、TXT / SRT / VTT エクスポートにも `氏名: テキスト` として反映されます
- 精度はモデル由来のベストエフォートです。声質が近い話者は同一視されることがあります。`SPEAKER_THRESHOLD`(既定 0.4)を上げると分かれやすく、下げるとまとまりやすくなります
- 無効化する場合は `DIARIZATION=0`。モデルが取得できない環境では自動的に話者タグなしで動作します

## 要約・議事録の生成(ローカル LLM 連携)

[Ollama](https://ollama.com/) / LM Studio / llama.cpp server など、**OpenAI 互換 API を持つローカル LLM サーバー**と連携して、文字起こし結果から要約・議事録を生成できます。`LLM_API_URL` を設定しない限り機能ごと無効で、UI にも表示されません(既定では外部にもローカルにも一切接続しません)。

```bash
# 例: ホスト側で Ollama を動かす場合
ollama pull qwen2.5:7b-instruct

# .env
LLM_API_URL=http://host.docker.internal:11434/v1   # Docker からホストの Ollama へ
LLM_MODEL=qwen2.5:7b-instruct
```

- ファイル文字起こしの完了後、結果画面の「要約・議事録」パネルから生成できます。結果はサーバーに保存され、再生成で上書きされます
- リアルタイム文字起こしでも、録音停止後に同じパネルから生成できます(こちらは保存されません)
- 生成物は Markdown でダウンロードできます。品質は使用する LLM モデルに依存します
- 議事録には概要・参加者・議論の要点・決定事項・TODO が含まれます(話者に氏名を設定しておくと参加者名に反映されます)
...
│   ├── llm.py         #   ローカル LLM 連携 (要約・議事録、任意機能)
│   ├── db.py          #   SQLite 永続化
│   ├── auth.py        #   認証の差し込みポイント (v1 は匿名)
│   └── config.py      #   環境変数ベースの設定
├── web/               # React + Vite フロントエンド
├── shared/            # サーバー・フロント共有の定義 (結合パラメータ)
├── tests/             # pytest スイート (モデル推論なし)
...
- Whisper はストリーミング非対応のため、リアルタイムは「VAD で発話の切れ目を検出 → 区間ごとに推論」する方式です。発話中は暫定テキスト(斜体)が逐次表示され、発話が終わってから 1〜数秒で確定テキストに置き換わります
...
- サーバーを再起動すると、処理待ち・処理中だったジョブは自動で再投入され、最初から処理し直されます
