...
- **中断(キャンセル)**: `POST /api/jobs/{id}/cancel` で待機中・処理中のジョブを打ち切れる。待機中はその場で `canceled` にし、処理中は `jobs.request_cancel()` がプロセス内メモリにフラグを立て、ワーカーが**次のセグメント境界**で `JobCanceled` を送出して止める(Whisper はセグメント境界でしか止められないため数秒の遅延がありうる)。**途中まで確定したセグメントは残す**(非破壊。確認・エクスポート可能)。元ファイルはワーカーが片付ける
